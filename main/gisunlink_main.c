	...
	while(1) {
		if(gisunlink_system->isConnectAp()) {
			...
			if(gisunlink_system->waitHWSn == false) {
				gisunlink_system->waitHWSn = getDeviceHWSnOrFirmwareVersion(GISUNLINK_HW_SN,gisunlink_system->deviceHWSn,true);
			}

			if(gisunlink_system->waitFirmwareVersion == false) {
				gisunlink_system->waitFirmwareVersion = getDeviceHWSnOrFirmwareVersion(GISUNLINK_FIRMWARE_VERSION,gisunlink_system->deviceFWVersion,true);
			}
#endif
